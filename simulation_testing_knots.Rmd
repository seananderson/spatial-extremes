...
n_knots = seq(10, 100, by = 10)
normality = 0
ratio_obs2pro = 2 # multiplier, e.g. obs error = 2 x process error would be value = 2
for(i in 1:length(n_knots)) {
  # simulate 1000 draws of posteriors -- with observation error equal to process error
  dataSim = simulateData(nKnots = n_knots[i])
  N = dim(dataSim$proj)[1] * dim(dataSim$proj)[2]
  SD = median(apply(dataSim$proj, 2, sd))*ratio_obs2pro
