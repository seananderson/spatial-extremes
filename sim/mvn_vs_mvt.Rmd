...
while (nu_big) {
  ...
  # fit the model
  time_dt <- system.time({stanMod = stan(file = 'stan_models/mvtSimulate.stan',data = spatglm_data,
  verbose = TRUE, chains = 1, thin = 1, warmup = 1000, iter = 2000) })
  #pars = spatglm_pars)
 
  time_mix <- system.time({spatglm_pars = c("gp_scale", "gp_sigmaSq", "scaledf")
  stanMod_trans = stan(file = 'stan_models/mvnSimulate_trans.stan',data = spatglm_data,
  verbose = TRUE, chains = 1, thin = 1, warmup = 1000, iter = 2000) })
  #pars = spatglm_pars)

  b_dt <- broom::tidy(stanMod, ess = T, estimate.method = "median", conf.int = T)
  dplyr::filter(b_dt, term == "scaledf")$ess / time_dt[[1]]
  min(b_dt$ess) / time_dt[[1]]

  b_mix <- broom::tidy(stanMod_trans, ess = T, estimate.method = "median", conf.int = T)
  dplyr::filter(b_mix, term == "scaledf")$ess / time_mix[[1]]
  min(b_mix$ess) / time_mix[[1]]

  dplyr::filter(b_dt, term == "scaledf")
  dplyr::filter(b_mix, term == "scaledf")
...
for(i in 1:nrow(df)) {
  ...
  # estimate model
  stanMod = stan(file = 'simulationTesting/mvtNormal_estSigma.stan',data = spatglm_data, 
...
for(i in 1:nrow(df_gamma)) {
  ...
  spatglm_pars = c("scaledf")
  # estimate model
  stanMod = stan(file = 'simulationTesting/mvtGamma_estSigma.stan',data = spatglm_data, 
  verbose = TRUE, chains = 1, thin = 1, warmup = 300, iter = 600, 
  pars = spatglm_pars)
