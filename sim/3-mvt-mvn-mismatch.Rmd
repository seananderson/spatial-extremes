...
simulate_and_fit <- function() {
  ...
  loo_t <- loo::loo(loo::extract_log_lik(m1$model))
  loo_n <- loo::loo(loo::extract_log_lik(m_wrong$model))
  
  i <<- i + 1
  list(i = i, dat = d, p = p, p_wrong = p_wrong, ess = ess, rhat = rhat, giveup = giveup,
    loo_t = loo_t, loo_n = loo_n)
}
```
