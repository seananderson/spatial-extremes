...
sim_and_fit <- function(N = 100, n_time = 15, slope = 0.5, plot = FALSE) {
  ...
  # Build our predictor variable:
  b <- sim_glmmfields(n_draws = n_time, gp_theta = 1.5, n_data_points = N,
    gp_sigma = 0.3, sd_obs = 0.1, n_knots = 10, df = 1000, g = locs)
  ...
  s <- sim_glmmfields(n_draws = n_time, gp_theta = 1.5, n_data_points = N,
    gp_sigma = 1.2, sd_obs = 0.9, n_knots = 10, X = X,
    B = c(0, slope), g = locs, df = 2) # B represents our intercept and slope
  d <- s$dat
  d$predictor <- b$dat$y
...
```{r simulate, cache=TRUE, dependson=c('sim_and_fit', 'initf')}
set.seed(1)
library(doParallel)
