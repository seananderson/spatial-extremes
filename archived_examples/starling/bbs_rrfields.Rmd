...
mvt_gamma = rrfield(sum ~ -1, data=d, time = "year", 
  ...
  estimate_df = TRUE, estimate_ar = FALSE, fixed_ar_value = 1, year_re = TRUE, algorithm="sampling", chains = 2L, iter=700, control = list(adapt_delta = 0.9))

mvn_gamma = rrfield(sum ~ -1, data=d, time = "year", 
  lon="Longitude", lat="Latitude", station = "StationID", nknots = 30, obs_error = "gamma", covariance="squared-exponential",prior_gp_sigma = half_t(100, 0, 0.5), prior_gp_scale = half_t(3, 0, 1), prior_intercept = student_t(100, log(mean(d[,"sum"])), 2), prior_sigma = half_t(3, 0, 0.5), 
  estimate_df = FALSE, estimate_ar = FALSE, fixed_ar_value = 1, year_re = TRUE, algorithm="sampling", chains = 2L, iter=700, control = list(adapt_delta = 0.9))
