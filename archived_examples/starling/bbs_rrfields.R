...
load("examples/starling/starling_rrfields.Rdata")
...
mvn_coverage = length(which(holdout[,"sum"] > mvn_pred$conf_low & holdout[,"sum"] < mvn_pred$conf_high))
mvt_coverage = length(which(holdout[,"sum"] > mvt_pred$conf_low & holdout[,"sum"] < mvt_pred$conf_high))
