...
\citet{latimer2009} provide an overview of predictive process
...
element described by a MVT random field. Our model describes the mean or expected
...
The symbol $\gamma$ describes the spatiotemporal process,
...
($\gamma_{s,t} = \phi \gamma_{s,t-1} + W_{s,t}$). In the last case,
...
computational stability and use a Gamma(2, 0.1) prior (i.e., shape = 2, scale = 0.1), 
...
to the data set representing 
mountain pine beetle outbreaks in the 
US Pacific Northwest (REF). We modeled the
proportion of grid cells affected by mountain pine beetle outbreaks
in the US states of XX and XX from 1994 to 2014.
This proportion could be modeled with a logit link and a
beta observation model, 
but since the maximum proportion affected was far from 1.0, we 
can instead fit a simpler model with a log link and lognormal observation model.
The log of the mean proportion affected location $s$ and time $t$, $\mu_{s,t}$,
...
\noindent with the spatial process modeled as an autoregressive process:

\begin{align}
    \gamma_{s,t} &= \phi \left(\gamma_{s,t-1} - \mathbb{E}[\gamma_{t-1}]\right) + W_{s,t},\\
     W_{s,t} &\sim \mathrm{MVT}\left(\nu, 0, \Sigma_{W(s,t)}\right).
 \end{align}
  
 \noindent Furthermore, we model the annual mean estimates, $\beta_t$, as following a random walk 
 constrained by $\sigma_t$, and the data $y_{s,t}$ to be generated by a lognormal observation model
 with scale parameter $\sigma_{\mathrm{obs}}$:
 
 \begin{align}
 \beta_t &\sim \mathrm{Normal}\left( \beta_{t-1}, \sigma_t \right),\\
  y_{s,t} &\sim \mathrm{LogNormal} \left(  \log(\mu_{s,t}), \sigma_{\mathrm{obs}} \right).
 \end{align}

We converted latitude and long and 2 degrees into UTMs and divided by $10^5$
to make the range of $\delta_{ij}$ approximately between 1 and 10 and therefore 
place X on a reasonable scale.
We chose 25 knots to represent the spatial process as described above.
We fit the models with Student-t(3, 0, 2) priors on \ldots
Student-t(3, 0, 5) on $\sigma_{\mathrm{obs}}$
Normal(0, 10) on $\beta_0$
Normal(0, 1)[-1, 1] on $\phi$

We fit two models to the data: (1) MVT random fields and (2) a model with MVN random
...
seasonal anomalies in the mean across the entire field). 

$\beta_t$ is the intercept for each time step, 
and $\sigma_{\mathrm{obs}}$ represents residual error. 
