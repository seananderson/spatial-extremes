...
% Perhaps it¿d be good if we also differentiated models that include spatial processes in the mean (e.g. a GAM) versus those that explicitly model spatial covariance, though this would have to get into obs vs process errors
...
lower uncertainty). We then illustrate the application of this modeling approach
to real-world data that includes spatial extremes, using data on mountain pine
beetle outbreaks in the Pacific Northwest of the United States. These
applications differ in their observation error model (normal, Gamma) and
treatment of temporal change, illustrating the flexibility of these robust
spatial models to other applications in ecology and related fields.
...
ecologists; here, we summarize the important elements of these models and
