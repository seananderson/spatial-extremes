...
\noindent with $\sigma^2 = 1$, $\eta = 1$, and spatial data drawn uniformly
...
can fit a model with a log link and lognormal observation model
(as opposed to a logit link and a beta observation model).
...
We converted latitude and longitude degrees into UTMs and divided by $10^5$
...
We fit the models with Student-t(3, 0, 2) priors on all scale parameters as described above,
a Normal(0, 10) prior on $\beta_0$,
a MVN(0, TODO) prior on $\gamma_{s,t=1}$,
and a Normal(0, 1)[-1, 1] prior on $\phi$.
