...
affected out-of-sample predictive accuracy and reduced predictive precision
...
interquartile range = XX; Figure~\ref{fig:recapture}e). TODO: something
summarizing the credible interval widths here? The leave-one-out information
criterion (LOOIC) correctly identified the MVT model in XX\% of the
simulations. 

The MVT random fields model generated more accurate and more precise
out-of-sample predictions for the mountain pine beetle data set
(Figure~\ref{fig:map-etc}). The degrees of freedom parameter, $\nu$, was
estimated at a value that indicates considerably heavy tails in the
autoregressive spatiotemporal process (median $\nu = XX$, 95\% CI = XX--XX,
Figure~\ref{fig:map-etc}c). The log predictive density (lpd) for held-out data
was higher for the MVT model compared to the MVN (median lpd = XX vs.\ XX,
Figure~\ref{fig:map-etc}d). Finally, 95\% credible intervals were on average XX
times wider for the MVN model compared to the MVT model
(Figure~\ref{fig:map-etc}e).
