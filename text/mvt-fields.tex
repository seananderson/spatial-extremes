...
$\nu = 2$), fitting a model with MVN random fields without extremes
reduced out-of-sample predictive accuracy and precision
(Fig.~\ref{fig:recapture}d--f). The misspecified model tended to overestimate
...
We have introduced a spatial process model that represents spatial random fields
through time with a MVT rather than MVN distribution. Through simulation, we
demonstrate that this new model can be accurately fit to data, although with
short time series, or considerable observation error, the model reverts to the
usual MVN model. Our simulations show that if the underlying true process
contains spatiotemporal extremes, our MVT model has superior predictive accuracy
and precision. Using a case study on mountain pine beetle outbreaks in the
Pacific Northwest, we show that our MVT model generates more accurate and
...
\citep[][]{rose1999} could generate apparent spatiotemporal extremes.
...
time. Additionally, human-caused disasters, such as marine oil spills could cause
...
not allow the spatial field to be characterized accurately, biasing
...
\citep[e.g.\ see][]{conn2015}. While the MVT random fields model is more robust than
...
efficiency over modelling a full covariance matrix describing all observed data
points. However, an important topic for research is to include sparse matrix
algorithms in spatiotemporal models of extreme events. A recent advance to
spatial models with very large datasets has been the stochastic partial
...
model introduced here is already reasonably efficient, accessible to a wide
