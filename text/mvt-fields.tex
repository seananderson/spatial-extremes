...
A limitation of spatial models that utilize Gaussian random fields is that they
...
Several extensions of Gaussian random fields have been proposed to better capture 
...
ecologists. For large datasets that include more than hundreds of locations, 
estimation of spatial effects at the locations of the data may be computationally 
prohibitive. One solution to this dimensionality problem is to estimate the random 
field as correlated random effects at a smaller subset of locations or $m$ 'knots' 
\citep[e.g.][]{latimer2009, shelton2014}, where $m < n$, the number of data points. 
...
effects at the locations of these knots, and known distance matrix between the knots 
...
our new MVT model differs from the usual MVN version. Both approaches are 
essentially GLMMs with a spatiotemporal element described by random field. 
The model describes the expected value of the response, $\mu
...
fields using a MVN distribution with some covariance matrix. Modifying this 
framework to the more flexible MVT distribution involves including one extra
degrees of freedom parameter $\nu$. When $\nu$ is small (say $\nu < 10$)
the distribution has considerably heavier tails than the MVN --- compared to the 
MVN, extreme events are more likely (Fig.~\ref{fig:nu}). As $\nu$ approaches infinity, 
...
% on zero.
The location of the $m$ knots describing a random field can be
estimated via a clustering algorithm, such as the partitioning around medoids
...
For demonstration, we use a squared exponential covariance function to model 
the covariance between knot locations. This covariance function, $H$, (also known as
the Gaussian covariance function) is one of the most commonly adopted choices of 
covariance functions and models the correlation
...
We used simulation testing to evaluate how well we could recover whether 
heavy tails were present in the spatial process under various conditions.
Our simulation included 50 data points collected at the same survey locations each year
for 5, 15, or 25 years. The spatial process was simulated as independent each
...
The degrees of freedom parameter $\nu$ was set to 2.5 (very heavy tails), 5 (moderately
heavy tails), or 20 (effectively normal tails). A final step in our data
simulation was to corrupt the ``true'' data by including
observation error. Observations were generated from a gamma
...
considerable measurement error. The underlying linear predictor was
