...
The elements of the covariance matrix $\bm{\Sigma}$ at the $m$ knot locations are
then defined as $\bm{\Sigma}_{ij}^* = \sigma^2 \left( \frac{-\delta_{ij}^2}{2 \eta}
...
$\bm{\Sigma}_{\left(\bm{W}, \bm{W^*} \right)}$
between the spatial random effects at the sample locations $\bm{W}$
and the realizations of the spatial process at the knot locations $\bm{W^*}$.
Given
$\bm{\Sigma^*}$, we can generate $\bm{W^*}$ by drawing
from a multivariate distribution (MVN or MVT) with covariance $\bm{\Sigma^*}$ and
projecting these to the data locations using $\bm{\Sigma}_{\left( \bm{W},\bm{W^{*}} \right)}$:
$\bm{W}=\bm{\Sigma}_{\left(\bm{W},\bm{W^*} \right)}^{'} \bm{\Sigma}^{*-1}\bm{W^*}$.
...
% \begin{align}
$\log(\bm{\mu_{s,t}}) \sim \mathrm{MVT}\left(\nu, \bm{0}, \bm{\Sigma_{W(s,t)}}\right),$
...
\begin{align} \log(\mu_{s,t}) &= \beta_t + \gamma_{s,t}, \label{eq:beetle-mu} \\
  ...
  \gamma_{s^{*},t} &\sim \mathrm{MVT}\left(\nu, \phi \gamma_{s^{*},t-1}, \bm{\Sigma}_{W(s^{*},t)}\right).
\end{align}
