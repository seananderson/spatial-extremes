...
model leads to better prediction (greater accuracy, more precision) when the spatial
...
heavy tails), or 20 (effectively normal tails). We then corrupted
the `true' spatial process with observation error, using a gamma
...
% TODO: make sure that all priors are listed in the supplement
% generating the simulated data. We placed weakly informative priors on $\sigma$,
% $\eta$, and CV of half-$t$(3, 0, 3) (i.e. Student-$t$(3, 0, 3)[0, $\infty$]). We

\subsection{Diagnosing the advantage of allowing for extremes}
\label{sec:diagnosing}

To evaluate the consequence of assuming spatial processes are not present when they really are,
...
Since the proportion affected was far from $1$, we
...
There are limitations to the approach we have described. First,
