...
spatiotemporal GLMMs without extremes.
\end{abstract}
...
function of distance. Our package allows for the isotropic squared exponential
(Gaussian), exponential, and Matern covariance functions; however, anisotropic
functions could be included. Given the random effects estimated at the knot
...
\citep[][Fig.~\ref{fig:didactic}]{latimer2009, finley2009}.
...
Our model is essentially a GLM with a spatiotemporal element described by a MVN
...
(say $\nu < 10$) the distribution has considerably heavier tails than the MVN
...
locations each year for 5, 15, or 25 years. We simulated the spatial process
...
heavy tails), or 20 (effectively normal tails). Our final step
was to corrupt the `true' process with observation error. We used a gamma
...
provide a full description of the posterior and joint probability distributions
...
distance relationship and divided the UTM values
