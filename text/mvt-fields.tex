...
We introduced a spatiotemporal model that is robust to spatial extremes through
time. Our model substitutes a MVT distribution for the usual MVN distribution
used in Gaussian Markov random field models.

What ecological and environmental processes could lead to spatiotemporal
extremes?

We expect the MVT random fields model to be widely useful in the fields of
ecology, environmental sciences, and fisheries. For example,

Our associated R package, \textbf{rrfields}, is flexible and includes
considerable additional features beyond those described thus far. For example,

To be sure, there are limitations to the approach described in this paper.
