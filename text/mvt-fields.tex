...
\left(- \delta_{ij}^2 / 2 \eta \right)$, where $\delta_{ij}$ is the
...
% TODO sigma^2?:
then defined as $\mathbf{\Sigma}_{ij}^* = \sigma^2 \exp \left( -\delta_{ij}^2 / 2 \eta
\right)$ with the spatial variance parameter $\sigma^2$ scaling the amplitude of the spatial
...
between the spatial random effects at the sample locations $\mathbf{W}$
and the realizations of the spatial process at the knot locations $\mathbf{W^*}$.
Given
...
projecting these to the data locations using $\mathbf{\Sigma}_{\left( \mathbf{W},\mathbf{W^{*}} \right)}$:
...
or MVT random field. We define the mean $\mu_{s,t} \equiv \mathbb{E}(y_{s,t})$ at a set of locations
in space $s$ and time $t$ as $g(\mu_{s,t}) = \mathbf{X}_{s,t} \bm{\beta} +
...
$\gamma_{s,t}$ represents the spatiotemporal process, described below. The variance of
...
with $\sigma = 0.3$ (the scale of the spatial deviations),
$\eta = 1.2$ (the special decay parameter),
...
\begin{figure}[htb]
  \begin{center}
    ...
    \caption{An illustration of three draws from a MVN random field (top row)
      ...
      The scale parameters $\sigma$, spatial decay parameters $\eta$,
...
\begin{figure}[htb]
  \begin{center}
    ...
    \caption{Simulation testing the MVT random fields model.
      ...
      $\sigma$ represents the spatial standard deviation,
      $\eta$ represents the spatial decay parameter,
...
\begin{multline}
  ...
  \times
  [\bm{\gamma}^*_{t} | \bm{\gamma}^*_{t-1}, \phi, \eta, \nu, \sigma^2] \\
  \times
  [\beta_{t=1}]
  [\bm{\gamma}^*_{t=1}]
  [\phi]
