...
$\nu = 2$), fitting a Gaussian (MVN) random fields without extremes reduced
out-of-sample predictive accuracy and precision
...
= \rmseWrong\ vs.\ \rmseRight; Figure~\ref{fig:recapture}e). The MVN model
prediction credible intervals were on average \meanMedianCIWiderSim \% larger
than the MVT model credible intervals. The leave-one-out information criterion
(LOOIC) correctly chose the MVT model in \looCorrectSim \% of the simulations.
