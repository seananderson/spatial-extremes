...
p <- data.frame(grid, v = (proj.norm), type = "projection")
d <- rbind(k, p)
...
```{r, fig.pos="placeHere", fig.cap = "Distribution of simulated data (single replicate)"}
# distance matrix of knots
distKnots = as.matrix(dist(knots))
distKnotsSq = distKnots^2 # squared distances
# note: shape parameter scaled to distance matrix
...
hist(y.binomial, 40, col="grey", xlab="Simulated data", main="")
```
...
```{r, fig.pos="placeHere", fig.cap="Simulated Poisson data (single replicate)"}
# distance matrix of knots
distKnots = as.matrix(dist(knots))
distKnotsSq = distKnots^2 # squared distances
# note: shape parameter scaled to distance matrix
...
```{r runJagsBinomialChunkRep, cache=TRUE}
jags.data = list("nLocs", "nKnots", "y.binomial", "distKnotsSq", "distKnots21Sq", "diagKnots", "muZeros", "indices", "nPoints")
   jags.params=c("pfat", "gp_sigmaSq_fat", "gp_sigmaSq_norm", "spatialEffectsKnotsNorm", "spatialEffectsKnotsFat")
jagsmodel.binomial.rep = jags.parallel(jags.data, inits=NULL, parameters.to.save=jags.params, model.file="recover_rf_binomial.txt", n.chains = 4, n.burnin=20000, n.thin=10, n.iter=30000, DIC=TRUE)
```
...
```{r, fig.pos="placeHere",fig.cap="Estimated contribution of wider tailed distribution for binomial data"}
attach.jags(jagsmodel.binomial, overwrite=TRUE)
...
```{r runJagsPoissonChunkRep, cache=TRUE}
jags.data = list("nLocs", "nKnots", "y.poisson", "distKnotsSq", "distKnots21Sq", "diagKnots", "muZeros", "indices", "nPoints")
jags.params=c("pfat", "gp_sigmaSq_fat", "gp_sigmaSq_norm", "spatialEffectsKnotsNorm", "spatialEffectsKnotsFat")
jagsmodel.poisson.rep = jags.parallel(jags.data, inits=NULL, parameters.to.save=jags.params, model.file="recover_rf_poisson.txt", n.chains = 4, n.burnin=20000, n.thin=10, n.iter=30000, DIC=TRUE)
```

Compare the densities of the estimated fat-tailed contribution between the model with a single replicate, and 5 years of data
```{r, fig.pos="placeHere",fig.cap="Estimated contribution of wider tailed distribution for Poisson data"}
attach.jags(jagsmodel.poisson, overwrite=TRUE)
par(mfrow = c(2,1), mgp=c(2,1,0), mai=c(0.5,0.6,0.4,0.05))
hist(pfat,40,col="grey",main="Single replicate")
