...
In this document we will simulate some data that come from a MVT random field and then it models that allow for the MVT random field or assume the standard MVN Gaussian random field. 

```{r set-knitr-options, cache=FALSE, echo=FALSE}
library("knitr")
opts_chunk$set(message=FALSE, fig.width=7, fig.height=5, cache = TRUE, autodep = TRUE)
```
...
ms1 <- group_by(d2, time) %>%
  mutate(rmse_wrong = sqrt(mean((est_wrong - proj) ^ 2)),
