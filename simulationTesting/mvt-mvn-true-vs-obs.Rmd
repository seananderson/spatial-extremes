...
simulate_and_fit <- function() {
  ...
  m1 <- rrfield(y ~ 0, data = s$dat, time = "time", station = "station_id",
    ...
    iter = ITER, chains = CHAINS, estimate_df = TRUE, save_log_lik = FALSE)
  
  m_wrong <- rrfield(y ~ 0, data = s$dat, time = "time", station = "station_id",
    lat = "lat", lon = "lon", nknots = nknots,
    iter = ITER, chains = CHAINS,
    estimate_df = FALSE, fixed_df_value = 10000, save_log_lik = FALSE)
...
```{r}
output <- plyr::rdply(.n = 8, simulate_and_fit)
output
```
