sim_mvt_rf <- function(n_pts = 100, n_knots = 30, n_draws = 20, gp_scale = 0.5,
  sigma_t = 0.2, mvt = TRUE, df = 3, seed = NULL,
  ...
  knots <- jitter(cluster::pam(grid, n_knots)$medoids)
  ...
  dist_all <- as.matrix(dist(rbind(grid, knots)))^2
