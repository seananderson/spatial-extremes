...
set.seed(1)
i <<- 0
output <- plyr::rlply(.n = 25, simulate_and_fit)
```
...
rmse
hist(rmse$perc_better, breaks = 8)
...
```{r}
# l1 <- loo::loo(loo::extract_log_lik(output[[3]]$m$model))
# l2 <- loo::loo(loo::extract_log_lik(output[[3]]$m_wrong$model))
# l1$looic
# l2$looic
# loo::compare(l1, l2)
```
