...
if (!file.exists(model_file)) {
  mvt <- rrfield(cover ~ 1, 
    ...
    prior_ar = half_t(1e9, 0, 0.3),
    estimate_ar = TRUE,
    estimate_df = TRUE,
    year_re = TRUE,
    chains = 4, iter = 800, thin = 1,
...
# ggplot(filter(combined, hold_out), aes((residual), color = model)) + geom_freqpoly(position = "identity") + facet_wrap(~year)
```
...
```{r}
# data_medians <- group_by(d, year) %>%
#   summarize(m = median(cover)) %>%
#   mutate(index = 1:n())
# 
# filter(bt, grepl("ar\\[", term))
# bs <- filter(bt, grepl("yearEffects", term))
# bs$index <- 1:nrow(bs)
# ggplot(bs, aes(index, exp(estimate), ymin = exp(conf.low), ymax = exp(conf.high))) +
#   geom_ribbon() +
#   geom_point(data = data_medians, aes(x = index, y = m), inherit.aes = FALSE)
```

Let's look at the coverage of the credible intervals between the 2 models:

```{r}
pred_mvt <- predict(mvt, interval = "prediction", conf_level = 0.9,
  newdata = d, type = "response")
pred_mvt <- pred_mvt %>% mutate(x = d$x, y = d$y, observed = d$cover, 
  year = d$year, hold_out = d$hold_out)

pred_mvn <- predict(mvn, interval = "prediction", conf_level = 0.9,
  newdata = d, type = "response")
pred_mvn <- pred_mvn %>% mutate(x = d$x, y = d$y, observed = d$cover, 
  year = d$year, hold_out = d$hold_out)
