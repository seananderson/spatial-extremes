...
g <- ggplot(subset(pred, hold_out), aes(x, y, fill = exp(estimate))) + 
  geom_tile() + facet_wrap(~year) +
  scale_fill_viridis(option = "D") +
  theme_light() +
  coord_fixed()
...
```{r}
mse <- subset(combined, hold_out) %>%
  group_by(model) %>%
  summarize(round(mean(residual^2), 2))
print(mse)
```

Let's look at the observed and predicted values with confidence intervals for 10 of the holdout data points. In the simulations we've shown that's the predictions in years with moderately large spikes can get inflated if we use the MVN model and the confidence intervals inflate to obtain the same correct coverage. 
