...
#'erdMYchla8day_LonPM180'
# grab integrated chlorophyll
chl = tabledap(info('siocalcofiHydroCasts'), fields = c('month','longitude', 'latitude', 'sta_id', 'sta_code', 'st_station','st_line','year','julian_day','intchl'))

chl = chl[-which(is.na(chl$intchl)),]
chl = chl[which(chl$year%in%c(1995:2008) & chl$month==4),]
d = chl
```
