...
td = td[td$month==4 & td$year%in%c(1989:2007), ]
...
library(dplyr)
g = group_by(td, station, year, common_name) %>% 
...
g = g[which(g$lon < -117.3 & g$lon > -125.6933 & g$lat > 29.83 & g$lat < 38.18),]

g = g[g$year %in% 1986:2002,]
ggplot(g, aes(lon, lat, color=log(div))) + geom_point(alpha=0.3) + facet_wrap(~year)
d = g
```
