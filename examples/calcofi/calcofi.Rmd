...
td = td[td$year%in%c(1994:2007) & td$month%in%c(4),]
...
g = group_by(td, station, year, common_name) %>% 
  summarize(x = mean(as.numeric(larvae_10m2)))
...
names(g)[which(names(g)=="x")]="density"
d = g
ggplot(g, aes(lon, lat, color = log(x))) + geom_point() + facet_wrap(~year)
