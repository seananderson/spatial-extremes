...
td = td[td$month==4 & td$year%in%c(1990:2002), ]

# try to calculate species diversity
simp.div = function(x) {
  1/sum((x/sum(x))^2)
}
g = group_by(td, station, common_name) %>% 
  summarize(x = sum(as.numeric(larvae_10m2))) %>% 
  group_by(time, station) %>% 
  summarize(div = simp.div(x))
  
td = left_join(td, g)


ggplot(td, aes(longitude, latitude, color = div)) + geom_point() + facet_wrap(~year)
