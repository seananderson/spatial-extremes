...
mvt_gamma = rrfield(sum ~ as.factor(year), data=d, time = "year", 
  lon="Longitude", lat="Latitude", station = "StationID", nknots = 30, obs_error = "gamma", covariance="squared-exponential",prior_gp_sigma = half_t(100, 0, 0.1), prior_gp_scale = half_t(10, 0, 0.1), prior_intercept = student_t(100, 3.6, 0.5), prior_sigma = half_t(10, 0, 0.5), prior_rw_sigma = half_t(10, 0, 0.3), prior_beta = student_t(30, 0, 1), 
  estimate_df = TRUE, estimate_ar = FALSE, fixed_ar_value = 1, year_re = FALSE, algorithm="sampling", chains = 4L, warmup = 1000, iter=1500, thin=1, control = list(adapt_delta = 0.99))

mvn_gamma = rrfield(sum ~ as.numeric(year), data=d, time = "year", 
  lon="Longitude", lat="Latitude", station = "StationID", nknots = 30, obs_error = "gamma", covariance="squared-exponential",prior_gp_sigma = half_t(100, 0, 0.1), prior_gp_scale = half_t(10, 0, 0.1), prior_intercept = student_t(100, 3.6, 0.5), prior_sigma = half_t(10, 0, 0.5), prior_rw_sigma = half_t(10, 0, 0.3), prior_beta = student_t(30, 0, 1), 
  estimate_df = FALSE, estimate_ar = FALSE, fixed_ar_value = 1, year_re = FALSE, algorithm="sampling", chains = 4L, warmup = 1000, iter=1500, iter=4000, thin=1, control = list(adapt_delta = 0.99))
