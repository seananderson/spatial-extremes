...
```{r}
d = read.csv("examples/sewardline/LTOP_Seward_merged_stages.csv")
...
d = d[,c("cruiseID","dateTime","stationID","consecStationNum","lat","lon","sonicDepth","towDepth","gear","ringDiam","meshSize","towType","chloropyllA")]
names(d)[which(names(d)%in%c("chloropyllA"))] = "density"
d = d[which(d$density > 0),]

d$year = as.numeric(substr(d$dateTime,1,4))
d$month = as.numeric(substr(d$dateTime,6,7))

# restrict data to may samples only -- continuous in most years
d = d[which(d$month%in%c(3,4,5) & d$year < 2007),]
```
...
stanMod_gamma = stan(file = 'stan_models/mvtGamma_estSigma_index.stan',data = spatglm_data, 
...
stanMod_normal = stan(file = 'stan_models/mvnGamma_estSigma_index.stan',data = spatglm_data, 
verbose = TRUE, chains = 3, thin = 1, warmup = 300, iter = 600, pars = spatglm_pars)
