...
Let's grab the chlorophyll bloom data for a selection of dates. We will start in January and grab every second week. This will be for 2014, which is the latest data available. 
...
dsub %>%
  ...
  facet_wrap(~month) +  
  scale_color_gradient2(low = RColorBrewer::brewer.pal(3, "PRGn")[[1]], high = RColorBrewer::brewer.pal(3, "PRGn")[[3]], mid = "grey97") +
  coord_fixed()
```
...
```{r plots}
pred <- predict(mvt, interval = "confidence")
...
```{r}
pred_mvn <- predict(mvn, interval = "confidence")
pred2 <- pred
