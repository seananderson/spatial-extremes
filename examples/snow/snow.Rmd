...
d = read.csv("examples/snow/snow.csv")
d = d[which(d$lon > -121 & d$lat < 40),]
d$ID = seq(1,nrow(d))
...
d$lat = d$lat/1000000
d$lon = d$lon/1000000
d$day = as.numeric(lapply(strsplit(as.character(d$date), "/"), getElement, 2))

d = d[which(d$change_snowdepth_cm>= 0),]
d$StationID = as.numeric(as.factor(d$station_id))

mvt_gamma = rrfield(change_snowdepth_cm ~ as.factor(day)-1, 
  data = d, 
  time = "day", lon = "lon", lat = "lat", station = "station_id", 
  nknots = 20, 
  obs_error = "nb2", 
...
mvt_gamma2 = rrfield(sum ~ as.factor(year) - 1, 
  data = d, 
  time = "year", lon = "Longitude", lat = "Latitude", station = "StationID", 
  nknots = 30, 
  obs_error = "gamma", 
  prior_gp_sigma = half_t(100, 0, 1),  # <- this line changed
  prior_gp_scale = half_t(100, 0, 1), 
...
mvn_gamma = rrfield(sum ~ as.factor(year), data=d, time = "year",
  lon="Longitude", lat="Latitude", station = "StationID", nknots = 30, obs_error = "gamma", covariance="squared-exponential",prior_gp_sigma = half_t(100, 0, 0.1), prior_gp_scale = half_t(100, 0, 0.05), prior_intercept = student_t(100, 3.5, 0.3), prior_beta = student_t(100, 0, 1), prior_sigma = half_t(10, 0, 0.5), 
  estimate_df = FALSE, estimate_ar = FALSE, fixed_ar_value = 1, algorithm="sampling", chains = 5L, warmup = 500, iter=1000, thin=1, control = list(adapt_delta = 0.99))
