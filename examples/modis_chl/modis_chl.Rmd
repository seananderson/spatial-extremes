...
# datasets here: https://coastwatch.pfeg.noaa.gov/erddap/search/index.html?page=1&itemsPerPage=1000&searchFor=chlorophyll

dataset = info('osuBloomsModisChla')
chl = griddap(dataset, time=c('2014-07-01T00:00:00Z','2014-07-30T00:00:00Z'), latitude=c(42,50), longitude=c(-127,-122))

chldata = chl$data %>% 
filter(round(lat,2) %in% c(seq(42,50,0.25))) %>% 
filter(round(lon,2) %in% c(seq(-127,-122,0.25))) %>%
  filter(!is.na(prc_chla))
ggplot(chldata,(aes(lon, lat,color=(prc_chla)))) + geom_point()

chldata = chl$data %>% 
filter(round(lat,2) > 46.26 & round(lat,2) < 48.5) %>% 
filter(round(lon,2) > -125.3 & round(lon,2) < -124) %>%
  filter(!is.na(prc_chla))
ggplot(chldata,(aes(lon, lat,color=(prc_chla)))) + geom_point(alpha=0.3,size=0.5)
